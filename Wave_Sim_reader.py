...
def GaussAnalyser(Matrix, TimePosition: int, ShowStar=False, sig0=60, Vacuum: bool = True):
    ...
    for tt in range(TimePosition+1, int(TimePosition+2*lambdt/dt/10)):
        ...
        for ii in range(1, w_count):
            SigMatBigTemp = np.vstack(
                (SigMatBigTemp, np.abs(Mat[sig_pos[ii], :, tt])))
    ...
    if Vacuum:
        ...
        popt, pcov = curve_fit(
            model_f2, x_ting, SigMat[0, :], p0=[sig0, 1, c00])
        c00 = popt[2]
        c_sig, c_peak, c_pos = np.array(0), np.array(0), np.array(0)
        c_sig, c_peak, c_pos = np.append(c_sig, (popt[0])), np.append(
            c_peak, popt[1]), np.append(c_pos, popt[2])
    ...
    for ii in range(1, w_count):
        ...
        if Vacuum:
            poptx, pcovx = curve_fit(model_f, x_ting, SigMat[ii, :], p0=[
                                     np.max(popt), 1], maxfev=5000)
        else:
            poptx, pcovx = curve_fit(model_f2, x_ting, SigMat[ii, :], p0=[
                                     np.max(c_sig), 1, c_pos[-1]], maxfev=5000)
            c_sig, c_peak, c_pos = np.append(c_sig, (poptx[0])), np.append(
                c_peak, poptx[1]), np.append(c_pos, poptx[2])
    ...
    map1 = ax1.pcolormesh(ylist, xlist, Matcapt,
...
def PlotTimePoint(Matrix, name, t: int, hundred: int, field, Pmatrix=None, Markline=None):
    ...
    map1 = plt.pcolormesh(
        linex, liney, Matrix[:, :, t], cmap='seismic', vmin=-1, vmax=1)
    cbar = plt.colorbar(map1)
...
def CutoffAnalyser(Matrix, TimePosition: int(), choicedx: float = dx, customname: str = ''):
    Mat = np.abs(Matrix)
...
def CMAanalysis(Matrix, density_matrix, cutoff: int, B0,
    ...
    map1 = ax1.pcolormesh(xlist, ylist, MatWithCutoff,
                          cmap='seismic', vmin=-1, vmax=1)
    cbar = plt.colorbar(map1)
    cbar.set_label(field + ' intensity')
    ax1.set_title(field + '-field at t = ' +
                  str(np.round((TimePosition+100*hundred)*dt*10**9, 3)/10**9) + 's')
    ax1.set_xlabel('y [m]')
    ax1.set_ylabel('x [m]')
    ...
    if Pmode == 'Linear':
        ...
        def blobmaker(x0, y0, signy, peak):
            ...
            for i in range(I-1):
                for j in range(J-1):
                    scaletemp[i, j] = peak * \
                        np.exp(-(x[i]-x0)**2/signy**2-(y[j]-y0)**2/signy**2)
            return scaletemp
...
def AnalyseBlobDispersion(Matrix, Wavepoint, plotname: str = 'Blob_plot.png', timepoint: int = 50):
    ...
    timerange = np.linspace(timepoint+1, timepoint +
                            int(Wavelength_time), int(Wavelength_time-1))
    ...
    if True:
        ...
        popt, pcov = curve_fit(model_b, liney, line, p0=[
                               guess_sig0l, guess_amp_l, guess_lx0, guess_sig0r, guess_amp_r, guess_rx0], maxfev=5000)
        perr = np.sqrt((pcov))[0]
...
def PlotDampenedBoundary(I, J, dmp_l, dampening_intensity):
    ...
    def dmp_model(dmp_mat, dmp_i):
     ...
     # For the sides
    Matrix[-dmp_l:, dmp_l:-
           dmp_l] = dmp_model(dmp_my, dmp_i) * Matrix[-dmp_l:, dmp_l:-dmp_l]
    Matrix[:-dmp_l, :dmp_l] = dmp_model(dmp_mx,
                                        dmp_i) * Matrix[:-dmp_l, :dmp_l]
    Matrix[:-dmp_l, -
           dmp_l:] = dmp_model(np.flip(dmp_mx, 1), dmp_i) * Matrix[:-dmp_l, -dmp_l:]

    # For the corners
    Matrix[-dmp_l:, -
           dmp_l:] = dmp_model(dmp_cnr, dmp_i) * Matrix[-dmp_l:, -dmp_l:]
    Matrix[-dmp_l:, :dmp_l] = dmp_model(dmp_cnl,
                                        dmp_i) * Matrix[-dmp_l:, :dmp_l]
