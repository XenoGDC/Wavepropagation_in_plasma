...
def WaveSim(TimeSize: int, Xsize: int, Ysize: int, Gauss_beam_FWHM: int, angle_frequency=O,
            ...
            Plasma_type='Vacuum', wave_polarity_angle=0, B0=[0, 0, 0.1], Linear_angle=0, dampening_length: int = 50, dampening_intensity: float = 7, blobintensity: float = 0.9e20, blobradius: float = 40,
            ...
            if Pmode == 'Constant':
                ...
                def lindensityincrease(lintheta, x0, y0, n0, matrix):
                    ...
                    for xx in range(len(matrix[:, 0])):
                        for yy in range(len(matrix[0, :])):
                            scaletemp[xx, yy] = n * \
                                (xx*np.cos(lintheta)+yy*np.sin(lintheta))
                    return scaletemp

                scale += lindensityincrease(lintheta,
                                            Linear_cutoff, J/2, 0.97e19, scale)
                ...
                def blobmaker(x0, y0, signy, peak):
                    ...
                    for i in range(I-1):
                        for j in range(J-1):
                            scaletemp[i, j] = peak * \
                                np.exp(-(x[i]-x0)**2/signy **
                                       2-(y[j]-y0)**2/signy**2)
                    return scaletemp

                scale += blobmaker(blobposition[0],
                                   blobposition[1], blobradius, blobintensity)
    ...
    dmp_my = np.flip(np.transpose(
        np.ones([1-2*dmp_l+J, dmp_l])*np.linspace(1, dmp_l, dmp_l)))
    ...
    for kk in range(T_big):
        ...
        for k in range(100):
            ...
            for t in range(intval):
                ...
                Ez[t + 1, 1, :-1] = np.cos(angle) * \
                    np.exp((-((y_ax - J / 2)*dy / sig) ** 2)) * sinf
                Ey[t + 1, 1, :-1] = np.sin(angle) * \
                    np.exp((-((y_ax - J / 2)*dy / sig) ** 2)) * sinf
                ...
                # For the sides
                Ex[t + 1, -dmp_l:, dmp_l:-
                    dmp_l] = dmp_model(dmp_my, dmp_i) * Ex[t + 1, -dmp_l:, dmp_l:-dmp_l]
                Ey[t + 1, -dmp_l:, dmp_l:-
                    dmp_l] = dmp_model(dmp_my, dmp_i) * Ey[t + 1, -dmp_l:, dmp_l:-dmp_l]
                Ez[t + 1, -dmp_l:, dmp_l:-
                    dmp_l] = dmp_model(dmp_my, dmp_i) * Ez[t + 1, -dmp_l:, dmp_l:-dmp_l]

                Ex[t + 1, :-dmp_l,
                    :dmp_l] = dmp_model(dmp_mx, dmp_i) * Ex[t + 1, :-dmp_l, :dmp_l]
                Ey[t + 1, :-dmp_l,
                    :dmp_l] = dmp_model(dmp_mx, dmp_i) * Ey[t + 1, :-dmp_l, :dmp_l]
                Ez[t + 1, :-dmp_l,
                    :dmp_l] = dmp_model(dmp_mx, dmp_i) * Ez[t + 1, :-dmp_l, :dmp_l]

                Ex[t + 1, :-dmp_l, -dmp_l:] = dmp_model(
                    np.flip(dmp_mx, 1), dmp_i) * Ex[t + 1, :-dmp_l, -dmp_l:]
                Ey[t + 1, :-dmp_l, -dmp_l:] = dmp_model(
                    np.flip(dmp_mx, 1), dmp_i) * Ey[t + 1, :-dmp_l, -dmp_l:]
                Ez[t + 1, :-dmp_l, -dmp_l:] = dmp_model(
                    np.flip(dmp_mx, 1), dmp_i) * Ez[t + 1, :-dmp_l, -dmp_l:]

                # For the corners
                Ex[t + 1, -dmp_l:, -
                    dmp_l:] = dmp_model(dmp_cnr, dmp_i) * Ex[t + 1, -dmp_l:, -dmp_l:]
                Ey[t + 1, -dmp_l:, -
                    dmp_l:] = dmp_model(dmp_cnr, dmp_i) * Ey[t + 1, -dmp_l:, -dmp_l:]
                Ez[t + 1, -dmp_l:, -
                    dmp_l:] = dmp_model(dmp_cnr, dmp_i) * Ez[t + 1, -dmp_l:, -dmp_l:]

                Ex[t + 1, -dmp_l:,
                    :dmp_l] = dmp_model(dmp_cnl, dmp_i) * Ex[t + 1, -dmp_l:, :dmp_l]
                Ey[t + 1, -dmp_l:,
                    :dmp_l] = dmp_model(dmp_cnl, dmp_i) * Ey[t + 1, -dmp_l:, :dmp_l]
                Ez[t + 1, -dmp_l:,
                    :dmp_l] = dmp_model(dmp_cnl, dmp_i) * Ez[t + 1, -dmp_l:, :dmp_l]

                # Check if the dampening is correct

                # Magnetic field
                Bx[t + 1, 1:-1, 1:-1] = Bx[t, 1:-1, 1:-1] - dt / \
                    dy * (Ez[t + 1, 1:-1, 2:] - Ez[t + 1, 1:-1, 1:-1])
                By[t + 1, 1:-1, 1:-1] = By[t, 1:-1, 1:-1] + dt / \
                    dx * (Ez[t + 1, 2:, 1:-1] - Ez[t + 1, 1:-1, 1:-1])
                ...
                # For the sides
                Bx[t + 1, -dmp_l:, dmp_l:-
                    dmp_l] = dmp_model(dmp_my, dmp_i) * Bx[t + 1, -dmp_l:, dmp_l:-dmp_l]
                By[t + 1, -dmp_l:, dmp_l:-
                    dmp_l] = dmp_model(dmp_my, dmp_i) * By[t + 1, -dmp_l:, dmp_l:-dmp_l]
                Bz[t + 1, -dmp_l:, dmp_l:-
                    dmp_l] = dmp_model(dmp_my, dmp_i) * Bz[t + 1, -dmp_l:, dmp_l:-dmp_l]

                Bx[t + 1, :-dmp_l,
                    :dmp_l] = dmp_model(dmp_mx, dmp_i) * Bx[t + 1, :-dmp_l, :dmp_l]
                By[t + 1, :-dmp_l,
                    :dmp_l] = dmp_model(dmp_mx, dmp_i) * By[t + 1, :-dmp_l, :dmp_l]
                Bz[t + 1, :-dmp_l,
                    :dmp_l] = dmp_model(dmp_mx, dmp_i) * Bz[t + 1, :-dmp_l, :dmp_l]

                Bx[t + 1, :-dmp_l, -dmp_l:] = dmp_model(
                    np.flip(dmp_mx, 1), dmp_i) * Bx[t + 1, :-dmp_l, -dmp_l:]
                By[t + 1, :-dmp_l, -dmp_l:] = dmp_model(
                    np.flip(dmp_mx, 1), dmp_i) * By[t + 1, :-dmp_l, -dmp_l:]
                Bz[t + 1, :-dmp_l, -dmp_l:] = dmp_model(
                    np.flip(dmp_mx, 1), dmp_i) * Bz[t + 1, :-dmp_l, -dmp_l:]

                # For the corners
                Bx[t + 1, -dmp_l:, -
                    dmp_l:] = dmp_model(dmp_cnr, dmp_i) * Bx[t + 1, -dmp_l:, -dmp_l:]
                By[t + 1, -dmp_l:, -
                    dmp_l:] = dmp_model(dmp_cnr, dmp_i) * By[t + 1, -dmp_l:, -dmp_l:]
                Bz[t + 1, -dmp_l:, -
                    dmp_l:] = dmp_model(dmp_cnr, dmp_i) * Bz[t + 1, -dmp_l:, -dmp_l:]

                Bx[t + 1, -dmp_l:,
                    :dmp_l] = dmp_model(dmp_cnl, dmp_i) * Bx[t + 1, -dmp_l:, :dmp_l]
                By[t + 1, -dmp_l:,
                    :dmp_l] = dmp_model(dmp_cnl, dmp_i) * By[t + 1, -dmp_l:, :dmp_l]
                Bz[t + 1, -dmp_l:,
                    :dmp_l] = dmp_model(dmp_cnl, dmp_i) * Bz[t + 1, -dmp_l:, :dmp_l]
                ...
                # For the sides
                Jx[t + 1, -dmp_l:, dmp_l:-
                    dmp_l] = dmp_model(dmp_my, dmp_i) * Jx[t + 1, -dmp_l:, dmp_l:-dmp_l]
                Jy[t + 1, -dmp_l:, dmp_l:-
                    dmp_l] = dmp_model(dmp_my, dmp_i) * Jy[t + 1, -dmp_l:, dmp_l:-dmp_l]
                Jz[t + 1, -dmp_l:, dmp_l:-
                    dmp_l] = dmp_model(dmp_my, dmp_i) * Jz[t + 1, -dmp_l:, dmp_l:-dmp_l]

                Jx[t + 1, :-dmp_l,
                    :dmp_l] = dmp_model(dmp_mx, dmp_i) * Jx[t + 1, :-dmp_l, :dmp_l]
                Jy[t + 1, :-dmp_l,
                    :dmp_l] = dmp_model(dmp_mx, dmp_i) * Jy[t + 1, :-dmp_l, :dmp_l]
                Jz[t + 1, :-dmp_l,
                    :dmp_l] = dmp_model(dmp_mx, dmp_i) * Jz[t + 1, :-dmp_l, :dmp_l]

                Jx[t + 1, :-dmp_l, -dmp_l:] = dmp_model(
                    np.flip(dmp_mx, 1), dmp_i) * Jx[t + 1, :-dmp_l, -dmp_l:]
                Jy[t + 1, :-dmp_l, -dmp_l:] = dmp_model(
                    np.flip(dmp_mx, 1), dmp_i) * Jy[t + 1, :-dmp_l, -dmp_l:]
                Jz[t + 1, :-dmp_l, -dmp_l:] = dmp_model(
                    np.flip(dmp_mx, 1), dmp_i) * Jz[t + 1, :-dmp_l, -dmp_l:]

                # For the corners
                Jx[t + 1, -dmp_l:, -
                    dmp_l:] = dmp_model(dmp_cnr, dmp_i) * Jx[t + 1, -dmp_l:, -dmp_l:]
                Jy[t + 1, -dmp_l:, -
                    dmp_l:] = dmp_model(dmp_cnr, dmp_i) * Jy[t + 1, -dmp_l:, -dmp_l:]
                Jz[t + 1, -dmp_l:, -
                    dmp_l:] = dmp_model(dmp_cnr, dmp_i) * Jz[t + 1, -dmp_l:, -dmp_l:]

                Jx[t + 1, -dmp_l:,
                    :dmp_l] = dmp_model(dmp_cnl, dmp_i) * Jx[t + 1, -dmp_l:, :dmp_l]
                Jy[t + 1, -dmp_l:,
                    :dmp_l] = dmp_model(dmp_cnl, dmp_i) * Jy[t + 1, -dmp_l:, :dmp_l]
                Jz[t + 1, -dmp_l:,
                    :dmp_l] = dmp_model(dmp_cnl, dmp_i) * Jz[t + 1, -dmp_l:, :dmp_l]
            ...
            if t_remain_min >= 1:
                print(str(int(round((100*kk+k)/T*100, 3))) + '% of simulation done. Est time remaining: ' +
                      str(t_remain_min) + 'min ' + str(t_remain_sec) + 'sec ', end='\r')
            else:
                print(str(int(round((100*kk+k)/T*100, 3))) +
                      '% of simulation done. Est time remaining: ' + str(t_remain_sec) + 'sec    ', end='\r')
        ...
        t2 = time.time()
        print('Computation time for timestamp ' +
              timestamp + ': ' + str(t2-t000) + 's')
