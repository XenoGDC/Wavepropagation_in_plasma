...
def GaussSingleAnalysis(Mat, timepoint: int, position: int, sig0: int = this_sig0, plotcurve=False):
    ...
    if plotcurve == True:
        ...
        plt.figure()
        plt.title('Gaussian curve {pos} m in'.format(
            pos=round(position*dx, 3)))
...
def Blobdispersion(File: str, timepoint: int, varmode: str = 'Density', sig0: int = this_sig0, plotname: str = 'Blob Dispersion', plottitle=''):
    ...
    for i in range(len(sims)):
        ...
        sig, amp, dsig, damp = GaussSingleAnalysis(
            matrix, timepoint_s, position=int(I*2/3), sig0=sig0)
        # print(sig)
    ...
    plt.ylabel('Apparent width at position ' +
               str(round(int(I*2/3)*rd.dx, 3)) + ' [m]')
